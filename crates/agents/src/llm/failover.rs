...
impl FailoverProvider {
    ...
    async fn try_complete(&self, request: LLMRequest) -> LLMResult<LLMResponse> {
        ...
        for index in 0..provider_count {
            ...
            info!(
                "[LLM-TRACE] Trying provider {} with timeout {}s",
                provider_name, self.config.attempt_timeout_secs
            );
            ...
            match result {
                Ok(Ok(response)) => {
                    info!(
                        "[LLM-TRACE] Provider {} succeeded in {:?}",
                ...
                Ok(Err(e)) => {
                    ...
                    warn!(
                        "[LLM-TRACE] Provider {} failed after {:?}: {}",
                        provider_name, provider_elapsed, e
                    );
                ...
                Err(_) => {
                    ...
                    warn!(
                        "[LLM-TRACE] Provider {} timed out after {:?} (limit: {}s)",
                        provider_name, provider_elapsed, self.config.attempt_timeout_secs
                    );
                    self.update_provider_health(index, false).await;
                }
            }
